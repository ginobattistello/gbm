...
def test_linear_gaussian_filter_matches_kalman(gaussian_filter_model):
    ...
    for yt in y:
        S = P + R
        lls.append(-.5*(np.log(2*np.pi*S) + (yt-(m+offset))**2/S))
        K = P/S
        m = m + K*(yt-(m+offset))
        P = (1-K)*P
...
def test_bernoulli_laplace_predictive_close_to_quadrature():
    ...
    model = StateModel(
        ...
        initial_state=[0.4], initial_state_covariance=[0.5], process_covariance=None,
    )
    out = model.evaluate([0,0], {"y": np.array([1]), "u": None})
    sd = np.sqrt(0.5); mu=.4
    f = lambda x: expit(x) * np.exp(-.5*((x-mu)/sd)**2)/(sd*np.sqrt(2*np.pi))
...
def test_categorical_filter_returns_psd_covariance():
    ...
    model = StateModel(
        lambda x,th,u,y:x, obs, "categorical",
        Priors(GaussianPrior([0],[0]), GaussianPrior([0],[0])),
        [0.2], initial_state_covariance=[.7]
    )
