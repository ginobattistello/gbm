...
from gbmtoolbox import Config, GaussianPrior, Priors, StateModel, bms, individual_fit, prior_sensitivity
...
def test_initial_covariance_scalar_vector_matrix():
    for spec, ref in [(0.2, 0.2 * np.eye(2)), ([0.2, 0.3], np.diag([0.2, 0.3])), ([[0.2, 0.05], [0.05, 0.3]], np.array([[0.2, 0.05], [0.05, 0.3]]))]:
        m = StateModel(lambda x, t, u, y: x, lambda x, p, u: 0.5, "bernoulli", _fixed_priors(), [0, 0], initial_state_covariance=spec)
...
def test_gaussian_defaults_to_estimated_observation_covariance():
    m = StateModel(lambda x, t, u, y: x, lambda x, p, u: [0.0], "gaussian", _fixed_priors(), [0])
...
def test_explicit_observation_covariance_overrides_default():
    m = StateModel(lambda x, t, u, y: x, lambda x, p, u: [0.0], "gaussian", _fixed_priors(), [0], observation_covariance=0.25)
...
def test_discrete_rejects_observation_covariance():
    with pytest.raises(ValueError, match="only used for Gaussian"):
        StateModel(lambda x, t, u, y: x, lambda x, p, u: 0.5, "bernoulli", _fixed_priors(), [0], observation_covariance=1.0)
...
def test_categorical_validation_requires_integer_y():
    model = StateModel(lambda x, t, u, y: x, lambda x, p, u: np.array([0.4, 0.6]), "categorical", _fixed_priors(), [0])
    with pytest.raises(ValueError, match="integer labels"):
        validate_fit_spec([{"y": np.array([0.0, 0.5]), "u": None}], model, Config())
...
def test_seed_reproducibility_for_bms():
    L = np.array([[1.0, 0.0], [2.0, 0.0], [1.5, 0.0]])
...
def test_callable_observation_covariance_uses_phi():
    model = StateModel(
        ...
        [0],
        observation_covariance=lambda phi, u: np.exp(2 * phi[0]),
    )
    R = model.observation_covariance_matrix(np.array([np.log(2.0)]), None, 1)
