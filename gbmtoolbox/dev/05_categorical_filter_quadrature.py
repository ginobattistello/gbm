...
def probs(x):
    z=np.array([x,0.,-x]); e=np.exp(z-z.max()); return e/e.sum()

model=StateModel(
    lambda x,th,u,y:x,
    lambda x,ph,u:jnp.asarray([x[0],0.,-x[0]]),  # logits; families.py applies the softmax
    "categorical",
    Priors(GaussianPrior([0],[0]),GaussianPrior([0],[0])),
    [.2],initial_state_covariance=[.7]
)
y=2
