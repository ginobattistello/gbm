...
bern = StateModel(lambda x, t, u, y: x, lambda x, p, u: float(np.log(0.8 / 0.2)), "bernoulli", fixed, [0])
...
cat = StateModel(lambda x, t, u, y: x, lambda x, p, u: jnp.asarray(np.log([0.2, 0.3, 0.5])), "categorical", fixed, [0])
out = cat.evaluate([0, 0], {"y": np.array([2, 1]), "u": None})["loglik"]
ref = np.log([0.5, 0.3])
print("Categorical max error:", np.max(np.abs(out - ref)))
assert np.allclose(out, ref, atol=1e-12)

gau = StateModel(lambda x, t, u, y: x, lambda x, p, u: jnp.asarray([0.0]), "gaussian", fixed, [0], observation_covariance=4.0)
out = gau.evaluate([0, 0], {"y": np.array([2.0]), "u": None})["loglik"][0]
ref = -0.5 * (np.log(2 * np.pi * 4) + 1.0)
