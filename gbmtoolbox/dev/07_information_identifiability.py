...
from gbmtoolbox import Config, GaussianPrior, Priors, StateModel, individual_fit, numerical_local_identifiability

model = StateModel(
    lambda x, th, u, y: x,
    lambda x, ph, u: ph[0] + ph[1],  # logit depends only on the sum
    "bernoulli",
    Priors(GaussianPrior([0], [0]), GaussianPrior([0, 0], [1, 1], names=["phi0", "phi1"])),
    [0],
)
